...
//@Component
//@Slf4j
//public class Mc implements InitializingBean {
//
//    @Value("${clientSecret}")
...
////        players=Mc.Players(max=20, online=1, sample=[Mc.Player(name=nooblong,
////        id=aa3d2f33-39dd-3074-b676-c243b240a809)]),
...
//        log.info("qq bot token 获取成功: {}", jsonResponse.toPrettyString());
...
//        Request post = OkUtil.post(objectNode, BASE_URL + "/v2/groups/" + qqGroupOpenId +
//                "/messages", header);
//
//        JsonNode jsonResponse = OkUtil.getJsonResponse(post, client);
...
//    public void check() {
//        try {
//            StatusResponse response = fetchData();
//            if (!open) {
//                sendMsg("服务器已开启: " + response.getDescription().getText() + "\n 地址: + " +
//                        mcUrl + ":" + mcPort);
//                open = true;
//            }
//            for (Player player : response.getPlayers().getSample()) {
//                if (!playTime.containsKey(player.getName())) {
//                    playTime.put(player.getName(), new Date());
//                    StringBuilder sb = new StringBuilder("当前服务器人数: " + response.getPlayers().getOnline() + "\n");
////                    sendMsg();
//                    break;
//                }
//            }
//        } catch (IOException e) {
//            log.error("获取mc服务器信息失败:", e);
//            if (open) {
//                sendMsg("服务器已关闭");
//                open = false;
//            }
...
//    }
//
//    @Scheduled(fixedDelay = 6000, timeUnit = TimeUnit.SECONDS, initialDelayString = "6000")
...
//        try {
//            this.setHost(new InetSocketAddress("cn-sz-yd-plustmp2.natfrp.cloud", 21942));
//            StatusResponse statusResponse = this.fetchData();
//            log.info("minecraft上线提醒启动: {}", statusResponse);
//            this.updateAccessToken();
//            this.sendMsg("服务启动！");
//        } catch (Exception e) {
//            log.error("minecraft上线提醒启动失败，忽略: ", e);
//        }
//    }
//
//    @Data
